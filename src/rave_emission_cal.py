...
filename = [
    ...
    if ("fire.pred.conus" in file) and (file.endswith(".nc"))
]
...
for i in np.arange(NN):
    ...
    var_frp = f.createVariable(
        "FRP_MEAN", "f4", ("time", "grid_yt", "grid_xt"), fill_value=-1
    )
    var_sd = f.createVariable(
        "FRP_SD", "f4", ("time", "grid_yt", "grid_xt"), fill_value=-1
    )
    var_fre = f.createVariable(
        "FRE", "f4", ("time", "grid_yt", "grid_xt"), fill_value=-1
    )
    var_emi = f.createVariable(
        "PM2.5", "f4", ("time", "grid_yt", "grid_xt"), fill_value=-1
    )
