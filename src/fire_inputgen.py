...
def main_driver(initial_hour, forecast_hour, f_input, f_output, lat_lim, lon_lim):
    ...
    if (index1[0] == 0) & (index2[0] == 0):
        ...
        data = readin['data'][index1[0]:index1[-1]+2, index2[0]-1:index2[-1]+2]
    elif index2[0] == 0:
        yt = yt[index1[0]-1:index1[-1]+2]
        xt = xt[index2[0]:index2[-1]+2]
        data = readin['data'][index1[0]-1:index1[-1]+2, index2[0]:index2[-1]+2]
    else:
        yt = yt[index1[0]-1:index1[-1]+2]
        xt = xt[index2[0]-1:index2[-1]+2]
    ...
    for j in np.arange(1, num+1, 1):
        ...
        if (loc[0]-fsize < 0) or (loc[0]+fsize+1 > XX) or (loc[1]-fsize < 0) or (loc[1]+fsize+1 > YY):
            skip = skip + 1
            continue
